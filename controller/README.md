...
4. **If the list is populated but the tile is black**  
   The minimal API **`GET /cameras/{id}/hls/index.m3u8`** returns **307** to MediaMTX at **`http://<API-host>:8888/{path}/index.m3u8`** (path matches `mediamtx.generated.yml`). Ensure **MediaMTX** is running (`./start.sh controller`). If the browser reaches the API on a different host than MediaMTX, set **`SMARTCAM_HLS_ORIGIN`** (e.g. `http://192.168.2.139:8888`) in the backend environment. **`VITE_HLS_BASE`** on the frontend should point at the same HLS origin for the direct fallback. Check **`GET /cameras/{id}/stream_health`** in `/docs` for hints.  
   **MediaMTX log `401 Unauthorized` on the RTSP source:** the camera is rejecting anonymous pulls. Set the camera **`url`** to include credentials, e.g. **`rtsp://admin:password@192.168.2.42:554/stream1`**, with the password **percent-encoded** if it contains `@`, `#`, `:`, etc. The YAML generator prints a **WARN** when the URL has no username; the dashboard settings form shows an amber hint in the same case.
