"""
Fan-out WebSocket messages to all connected ``/ws/detections`` clients.
    ...
    async def broadcast(self, payload: Dict[str, Any]) -> None:
        ...
        for ws in targets:
            try:
                ...
                logger.debug("detections ws send failed: %s", e)
                dead.append(ws)
        if dead:
            async with self._lock:
                for ws in dead:
...
_hub: Optional[DetectionsHub] = None


def get_detections_hub() -> DetectionsHub:
    global _hub
    if _hub is None:
        _hub = DetectionsHub()
    return _hub
